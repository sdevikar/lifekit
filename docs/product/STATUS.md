...
| Step 13a feed API | ✅ Shipped 2026-09-22 — `lifekit.serve.server` (Flask, localhost-only `:8765`): `GET /api/briefing/today`, `POST /api/completions`, `POST /api/idea-signals`, conversations CRUD + `POST /api/conversations/:id/messages` (grounded chat via retrieval + `lifekit.llm`). Schema migration adds `conversations`, `messages` tables; `idea_signals` column added to completions. Suite 112/112 (82 + 30 new feed API tests). Verified against dogfood DB (`~/.lifekit/lifekit.db`): briefing returns exercise 28 + stages, completions persist, conversation round-trip works. Slice archive at `../../openspec/archives/step-13a-feed-api/`. |
| Step 13b feed screen | ✅ Shipped 2026-09-23 — Next.js 16.3.6 (App Router, TS, Tailwind) in `frontend/`: 7 feed components (TodayHeader, ExerciseCard, ResurfacedIdeaCard, FadingIdeasCard, ConversationsList, MasterComposer, conversation stub at `/c/[id]`); `/api/*` rewrites proxy to 13a feed API; `useBriefing` hook wires all actions. `npx next build` passes; all API actions verified through proxy against dogfood DB. Slice archive at `../../openspec/archives/step-13b-feed-screen/`. |
| Step 13c conversation view | ✅ Shipped 2026-09-23 — Conversation screen at `/c/[id]`: "‹ Feed" back button, title, seed badge, MessageList (user/coach bubbles with timestamps), ChatInput (pinned bottom, optimistic rendering). `useConversation` hook loads via `GET /api/conversations/:id`, sends via `POST /api/conversations/:id/messages`. Full loop verified against dogfood DB: card seed → message → grounded reply → 2 messages persisted. Build + tsc pass. Slice archive at `../../openspec/archives/step-13c-conversation/`. |
| Step 13d serve command | ✅ Shipped 2026-09-24 — `lifekit ui` (Python CLI) starts the 13a feed API (`:8765`) + Next.js production build (`:3000`), localhost only, clean shutdown via SIGTERM/SIGINT. Streamlit UI removed from `ui/app.py` + pyproject.toml + devcontainer. `npx next start` proxy verified: briefing, completions, conversations all work through `/api/*` rewrite. Slice archive at `../../openspec/archives/step-13d-serve-command/`. |
| Phone access (workstation) | 📋 Updating — `uv sync`, copy `dogfood/lifekit.db` → `~/.lifekit/lifekit.db`, build frontend (`cd frontend && npx next build`), then `lifekit ui`; phone opens the workstation's tailnet IP on port **3000** (was 8501 Streamlit). Devcontainer updated (`.devcontainer/devcontainer.json`). |
...
| Intent contract + book model proposal | ✅ Done 2026-09-20 — `../../intent.md` (standing product/SDLC contract derived from `PHILOSOPHY.md`, written for the coding agent), `../../intent/` folder with `_template.md` (per-change proto-specs per the Claude Academy "Capture as intent.md" practice: draft → PO review → human approval → Design), and `../../AGENTS.md` (instructions for OpenCode on the intent workflow, since OpenCode has no native intent-file support). `../backend/BOOK_MODEL.md` proposes the book-model shape: structure tree + typed items + relations + quote grounding, minimal v1 SQLite schema, structuring-pass build path. Awaiting human review; dogfood exit criterion still undecided (gate). |
...
- **2026-09-24** — Step 13d serve command shipped: `lifekit ui` starts the feed API on `:8765` + Next.js production server (`:3000`), localhost only, clean shutdown. Streamlit removed, `.devcontainer` updated. `npx next start` proxy verified. **Step 13 complete.**
- **2026-09-23** — Step 13c conversation view shipped: conversation screen at `/c/[id]` with "‹ Feed" back button, title, seed badge, MessageList (user/coach bubbles), ChatInput (pinned bottom, optimistic rendering). `useConversation` hook loads via `GET /api/conversations/:id` and sends via `POST /api/conversations/:id/messages`. Full loop verified against the dogfood DB: card seed → message → grounded reply → 2 messages persisted. Slice archived at `openspec/archives/step-13c-conversation/`.
- **2026-09-23** — Step 13b feed screen shipped: Next.js 16.3.6 (App Router, TS, Tailwind) in `frontend/`. 7 feed components per the umbrella's visual spec (TodayHeader, ExerciseCard, ResurfacedIdeaCard, FadingIdeasCard, ConversationsList, MasterComposer, conversation stub at `/c/[id]`). `/api/*` rewrites proxy to the 13a feed API (`:8765`). `useBriefing` hook wires Mark done, Still with me, and composer → conversation creation. `npx next build` passes; all API actions verified through proxy against the dogfood DB. Slice archived at `openspec/archives/step-13b-feed-screen/`.
